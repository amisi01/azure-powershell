...
{
    [Cmdlet("Restore", ResourceManager.Common.AzureRMConstants.AzureRMPrefix + "CosmosDBMongoDBCollection", DefaultParameterSetName = NameParameterSet, SupportsShouldProcess = true), OutputType(typeof(PSMongoDBCollectionGetResults), typeof(ConflictingResourceException))]
    public class RestoreAzCosmosDBMongoDBCollection : AzureCosmosDBCmdletBase
    {
        ...
        public override void ExecuteCmdlet()
        {
            ...
            try
            {
                readMongoDBCollectionGetResults = this.CosmosDBManagementClient.MongoDBResources.GetMongoDBCollection(this.ResourceGroupName, this.AccountName, this.DatabaseName, this.Name);
            }
            ...
            if (this.ShouldProcess(this.Name, "Restoring MongoDB Collection"))
            {
                MongoDBCollectionGetResults mongoDBCollectionGetResults = this.CosmosDBManagementClient.MongoDBResources.CreateUpdateMongoDBCollectionWithHttpMessagesAsync(this.ResourceGroupName, this.AccountName, this.DatabaseName, this.Name, mongoDBCollectionCreateUpdateParameters).GetAwaiter().GetResult().Body;
                this.WriteObject(new PSMongoDBCollectionGetResults(mongoDBCollectionGetResults));
            }
